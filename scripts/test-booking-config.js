  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.source === 'fallback') {
        ...
        console.log('   2. BOOKINGS_SERVICE_ID or BOOKINGS_STAFF_ID is incorrect');
...
async function testZohoBookingsToken() {
  ...
  try {
    ...
    const response = await fetch('https://ideinstein.com/api/debug/zoho-tokens?service=bookings', {
      headers: {
